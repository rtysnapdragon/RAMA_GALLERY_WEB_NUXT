...
const aiResponses: Record<string, string> = {
  default: tBy({en:"Great question! RamaGallery features hundreds of artworks by Cambodia's finest artists — from traditional Apsara paintings to cutting-edge digital creations. Try browsing the Gallery or searching for a specific style!", km:"សួស្តី! ខ្ញុំជា RamaAI — ជំនួយការ AI របស់ RamaGallery។ ខ្ញុំអាចជួយអ្នករុករកវិចិត្រសាល ណែនាំស្នាដៃ ឬឆ្លើយសំណួររបស់អ្នកបាន!"}),
  artwork: tBy({en:"I recommend checking out 'Apsara in Gold' by Sophea Khun — it's one of our most celebrated pieces. You can also filter by category in the Gallery page!", km:"ខ្ញុំសូមណែនាំឲ្យអ្នកពិនិត្យមើល 'Apsara in Gold' ដោយ Sophea Khun — វាជាស្នាដៃដ៏ល្បីល្បាញមួយរបស់យើង។ អ្នកក៏អាចត្រងតាមប្រភេទនៅក្នុងទំព័រវិចិត្រសាលបានដែរ!"}),
  artist: tBy({en:"Some of our top artists include Sophea Khun (painting), Dara Meng (photography), and Pisey Ros (digital art). Visit the Artists page to see their full portfolios!", km:"ខ្ញុំសូមណែនាំឲ្យអ្នកពិនិត្យមើល 'Apsara in Gold' ដោយ Sophea Khun — វាជាស្នាដៃដ៏ល្បីល្បាញមួយរបស់យើង។ អ្នកក៏អាចត្រងតាមប្រភេទនៅក្នុងទំព័រវិចិត្រសាលបានដែរ!"}),
  join: tBy({en:"Joining is easy! Click 'Join' in the header to create an account. As an artist, you'll get a full portfolio page, AI tools, analytics, and access to our collector network.", km:"ខ្ញុំសូមណែនាំឲ្យអ្នកពិនិត្យមើល 'Apsara in Gold' ដោយ Sophea Khun — វាជាស្នាដៃដ៏ល្បីល្បាញមួយរបស់យើង។ អ្នកក៏អាចត្រងតាមប្រភេទនៅក្នុងទំព័រវិចិត្រសាលបានដែរ!"}),
}
...
const sendMessage = async (text?: string) => {
  ...
  try {
    ...
    const response = await useWeb<any>('api/ai/chat', {
      method: 'POST',
      data: { 
        message: content,
    ...
    // Fallback to default mock responses if API fails or doesn't exist
